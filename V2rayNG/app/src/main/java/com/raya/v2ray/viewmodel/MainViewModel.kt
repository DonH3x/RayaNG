package com.raya.v2ray.viewmodel
...
import com.raya.v2ray.AngApplication
import com.raya.v2ray.AppConfig
import com.raya.v2ray.AppConfig.FAST_CONNECT_PING_THRESHOLD
import com.raya.v2ray.AppConfig.FAST_CONNECT_PREFERRED_PATTERN
import com.raya.v2ray.R
import com.raya.v2ray.dto.ProfileItem
import com.raya.v2ray.dto.ServersCache
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
  ...
  val startBestServer by lazy { MutableLiveData<String>() }
  private var bestServer: Pair<String, Long>? = null
  private var testedServerCount: Int = -1
...
//            }
   ...
   */
  fun testAllRealPing(selectBest: Boolean = false) {
    ...
    viewModelScope.launch(Dispatchers.Default) {
      for (item in serversCopy) {
        if (selectBest)
          MessageUtil.sendMsg2TestService(getApplication(), AppConfig.MSG_MEASURE_CONFIG_NUI, item.guid)
        else
  ...
  /**
   * Tests real ping for all servers and selects the one with best ping.
   */
  fun selectBestServer() {
    testedServerCount = 0
    startBestServer.value = ""
  ...
  private val mMsgReceiver = object : BroadcastReceiver() {
    override fun onReceive(ctx: Context?, intent: Intent?) {
      when (intent?.getIntExtra("key", 0)) {
        ...
        AppConfig.MSG_MEASURE_CONFIG_NUI_SUCCESS -> {
          if (startBestServer.value.isNotNullEmpty()) {
            return
          }
          testedServerCount++
          val resultPair = intent.serializable<Pair<String, Long>>("content") ?: return
          if (resultPair.second >= 0) {
            ...
            if (bestServer == null || resultPair.second < bestServer!!.second)
              bestServer = resultPair
          }

          if (testedServerCount == serversCache.toList().size) {
            startBestServer.value = if (bestServer != null)
              bestServer!!.first
            else
              null
          }
